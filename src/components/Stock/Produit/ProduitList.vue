...
<div>
        <div>    
            <div class="d-md-flex m-3 justify-content-between" >
                <button class="btn btn-info mt-5 mb-5 ml-5" @click="modalActive = true,$store.state.IdEditProduit=null">
                        ...
                        <table class="table table-bordered  table-striped table-hover text-center" id="dataTable" width="100%" cellspacing="0">
                            ...
                            <tbody>
                               <tr v-for="produit in produits" :key="produit.id">
                                ...
                                <td>{{ produit.nom }} </td>
                                <td>{{ produit.caisse }} </td>
                                <td>{{ produit.uniteDeMesure }} </td>
                                <td>{{ produit.nbBouteille }} </td>
                                <td>
                                    <button class="btn btn-sm btn-default m-2"  @click="deleteProduit(produit.id)"><font-awesome-icon icon="fa-solid fa-trash"/>
                                    </button>
                                    <button class="btn btn-sm btn-default" @click="modalActive = true,editProduit(produit,produit.id)" >
...
export default {
    ...
    computed:{
        searchEvery(){
            return this.produits.filter(val=>val.includes(this.search))
            }
    },
    methods:{
        fetchData() {
        ...
        deleteProduit(id) {
            axios.delete(this.$store.state.baseUrl + "/produits/" + id)
            .then(resp => {
                this.produits = resp.data
                this.fetchData()
            })
        ...
        editProduit(produit,id){
        this.$store.state.IdEditProduit=id
        this.$store.state.produits=produit
